...
const axios = require('axios'); // Axios to call Flask API
...
// Call Python Flask API to predict fraud
async function predictFraud(features) {
    try {
        const response = await axios.post('http://localhost:5000/predict', {
            features: Object.values(features) // Send features to Flask API
        });
        return response.data.prediction[0]; // Get the prediction (0 or 1)
    } catch (error) {
...
async function processNewBlock(blockNumber) {
    try {
        ...
        for (const tx of block.transactions) {
            ...
            const isSenderFraudulent = await predictFraud(senderFeatures);
            const isReceiverFraudulent = await predictFraud(receiverFeatures);
